...
class RoomHandler:
    ...
    async def switch_brackets(self, playing: bool) -> None:
        if not self.current_room:
            raise ValueError("Not in a room")
        await self.client.send_message("room.bracket.switch", {
            "content": "player" if playing else "spectator"
        })

    async def handle_join(self, data: Dict[str, Any]) -> None:
        """Handle room join events"""
        print(f"Joined room: {data}")
        await self.client.emit("client.room.join", data)
